---
icon: material/numeric-6-box
title: Cvičení 6
---
<div style="text-align: center;" markdown="1">

## formulář odešlete nejpozději do čtvrtka 9. dubna do 12.00!

</div>

<figure markdown>
![](./assets/data/155PKAR – 2. kontrola.png){ width=200px }
</figure>

[<span>ArcGIS Survey123</span><br>Druhý kontrolní formulář](https://arcg.is/1zaSHO1){ .md-button .md-button--primary .server_name .external_link_icon_small target="_blank"}
{: .button_array}

[Časový harmonogram](https://docs.google.com/spreadsheets/d/1CRaESwVAKTbf1wrZwxh72D2qE0wl7teVPnMgxCTqiw8/edit?gid=1862868462#gid=1862868462){ .md-button .md-button--primary .server_name .external_link_icon_small target="_blank"}
{: .button_array}